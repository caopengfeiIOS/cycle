...
- (NSString *)UTF8{
    ...
    NSString *newStr=[[NSMutableString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return newStr;
}
/**
 *  地址只保留地市
...
- (NSString *)proCityMsg{
    ...
    NSArray *arr = [self componentsSeparatedByString:@"市"];
    if (arr.count>1) {
        
        return [NSString stringWithFormat:@"%@市",arr.firstObject];
    }
    return self;
}

+ (NSString *)KilometerWithNumber:(NSNumber *)num{
    
    float NewNum = num.floatValue/1000;
